 ...
 * pour installer la bibliotheque uvsq en trez dans le dossier uvsqgraphics et ouvrez le terminal dedans et tapez "make"
 
 *tappez dans le terminal la station de depart et darrivée et regarder dans le terminal pour voir littéraire a prendre
 * */
...
graph loadGraph (char *nomfichier)
{
  ...
  for (i = 0; i < G.arcs; i++)
    {
      fscanf (fp, "%d%d%d", &G.matrice[i].depart, &G.matrice[i].destination,
	      &G.matrice[i].temps);//lecture d'une ligne depart arrive temps 
...
int floydWarshall (graph g, Station S)
{
  ...
  for (i = 0; i < g.arcs; i++)
    tempsAret[g.matrice[i].depart - 1][g.matrice[i].destination - 1] = g.matrice[i].temps;
	...
	if (i != j)
	  {
	    ...
	    afficheLignes (S, g);
	    printf ("Vous allez partir de:%s vers :%s\n temps estimee %3d \n",
		    S.nomStation[i + 1], S.nomStation[j + 1],
		    tempsAret[i][j]);
	    POINT p;

	    p.x = S.cord_x[i + 1];
	    p.y = S.cord_y[i + 1] + 10;
	    aff_pol ("Depart", 10, p, blanc);
	    p.x = S.cord_x[j + 1];
	    p.y = S.cord_y[j + 1] + 10;
